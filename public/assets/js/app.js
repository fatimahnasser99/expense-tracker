...
function setHtmlValues() {
  ...
  expenseDiv.innerHTML = expense; //add classes    
  ...
  balanceDiv.className = 'text-4xl'; //insert to html file
...
function updateHistory() {
  ...
  if (amount[pos] > 0) dv2.classList.add("border-r-2", "border-green-600", "px-2");
  if (amount[pos] < 0) dv2.classList.add("border-r-2", "border-red-600", "px-2");
  containerpos[pos].append(dv2);
}

function updateWallet() {
  var size = amount.length;
  if (amount[size - 1] > 0) income += amount[size - 1];
  if (amount[size - 1] < 0) expense += amount[size - 1];
